// Classroom Challenge — app.js (CORE) v19.1.8-core
// Core responsibilities only:
// - config + globals
// - auth (id → pin → login/register)
// - core data fetch (activities, grading, submissions, profile)
// - render: profile, activities, archive, leaderboard (basic)
// - submit flow with quiet profile refresh
// - panel-scoped busy setup
// Non-core UI sugar (archive toggle, leaderboard toggle, avatar UI) will live in separate files.
...
let lastLeaderboardRefresh = 0; // for throttling after submit
...
function saveTimestamp(key) {
  ...
  try { localStorage.setItem(K.ts, JSON.stringify(cache.timestamps)); } catch {}
}
...
function renderProfile(p) {
  ...
  // NOTE: avatar is rendered by app-avatar-patch.js via window.__profile
}
...
function renderLeaderboard(rows, opts = {}) {
  ...
  (rows || []).slice(0, limit).forEach((r, i) => {
    ...
    // avatar in leaderboard will be added by avatar patch if needed
...
function renderArchive() {
  ...
  if (!cache.archive.length) {
    ul.innerHTML = '<li class="muted">No previous activities yet.</li>';
    return;
  }
  cache.archive.forEach(item => {
    const li = document.createElement('li');
    li.className = 'archive-item';
    li.dataset.id = item.activityId;
...
function renderDash() {
  ...
  // expose for other scripts (avatar, etc.)
...
async function goToPin(mode) {
  ...
  try {
    ...
    // try to show leaderboard preview if stale
    ...
    setMsg('#idMsg', e.message || 'Network error');
    showAuthId();
  } finally {
    await Busy.hide();
  }
...
async function onPrimaryAuth() {
  ...
  try {
    ...
    // capture basic profile (avatar will be handled by avatar file)
...
async function onSubmit(ev) {
  ...
  if (!answer) {
    const t = tileEl(id);
    showTileToast(t, 'Enter an answer', 'info');
    return;
  }
  ...
  removeClasses(tile, 'success', 'fail', 'neutral', 'done');
  addClasses(tile, 'processing', 'locked');
  ...
  try {
    ...
    if (entry && entry.hash) {
      ...
      if (h === entry.hash) { verdict = 'success'; points = Number(entry.points || 0); }
      else { verdict = 'fail'; }
    }
  } catch { }
  ...
  if (verdict === 'success') {
    ...
    showTileToast(tile, '❌ Not quite — recorded.', 'bad');
  } else {
    showTileToast(tile, 'ℹ️ Submitted.', 'info');
  }
  ...
  try {
    ...
    btn.disabled = false;
    showTileToast(tile, e.message || 'Error', 'bad');
  }
}
...
// expose core helpers for feature files
window.CC = {
  fetch: { get: jget, post: jpost },
  busy: window.CCPanelBusy,
  toast: showToast,
  cache,
  store,
  setProfile(p) {
    cache.profile = p;
    window.__profile = p;
    try { localStorage.setItem(K.prof, JSON.stringify(p)); } catch {}
  }
};
